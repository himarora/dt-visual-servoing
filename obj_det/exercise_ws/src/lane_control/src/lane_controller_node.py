...
class LaneControllerNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        self.obj_det_sub = rospy.Subscriber("/agent/obj_det_node/obj_dets",
                                                 SegmentList,
                                                 self.obj_det_cb,
                                                 queue_size=1)
        
        self.no_pedestrian = True
        self.pedestrian_info = (None, None, None, None, None)
        self.log("Initialized!")
        
    def obj_det_cb(self, obj_dets):
        segments = obj_dets.segments
        ...
        def get_dets():
            for segment in segments:
                x_min = segment.pixels_normalized[0].x
                y_min = segment.pixels_normalized[0].y
                x_max = segment.pixels_normalized[1].x
                y_max = segment.pixels_normalized[1].y
                label = segment.color
                yield x_min, y_min, x_max, y_max, label
        
        for x_min, y_min, x_max, y_max, label in get_dets():
            # TODO identify the detection that's the most likely to be an pedestrian avoidance task
            # this might mean that there are no pedestrian to avoid here, for example if all detections are on the edge of the road
            # a good heuristic would be "if detection is about in the middle of the image and measures more than X pixels, assume
            # that that's a duckie on the road"
            # TODO if no such detection is found, don't forget to set self.no_pedestrian to True!
            # TODO self.pedestrian_info = a problematic detection that we want to avoid
            # TODO self.no_pedestrian = False 
            pass
    ...
    def getControlAction(self, pose_msg):
        ...
        # TODO check if there's an obstacle to avoid, and if yes, avoid it using the information from self.pedestrian_info
